// learning-embed — Generate Gemini embeddings for learning_examples rows
// Pattern: same Gemini embedding as bank-embed-transactions, postgres npm for halfvec update
...
/* ─── Gemini embedding (same as bank-embed-transactions) ── */
...
/* ─── concurrency helper (same as bank-embed-transactions) ── */
...
Deno.serve(async (req) => {
  ...
  try {
    let rows: { id: string; input_text: string }[];

    if (body.mode === "backfill") {
      // Backfill mode: fetch unembedded rows
      const batchSize = Math.max(1, Math.min(body.batch_size || 50, MAX_BATCH));
      rows = await sql`
        SELECT id, input_text FROM public.learning_examples
        WHERE company_id = ${companyId}
          AND embedding IS NULL
          AND length(input_text) > 3
        ORDER BY created_at
        LIMIT ${batchSize}
      `;
    } else {
      // Explicit IDs mode
      const ids = Array.isArray(body.example_ids)
        ? body.example_ids.filter((v): v is string => typeof v === "string" && v.length > 0).slice(0, MAX_BATCH)
        : [];
      if (ids.length === 0) return json({ error: "example_ids vuoto" }, 400);
    ...
    if (rows.length === 0) {
      // Count remaining for backfill mode
      const [{ count: remaining }] = await sql`
        SELECT count(*)::int as count FROM public.learning_examples
    ...
    await runWithConcurrency(rows, EMBED_CONCURRENCY, async (row) => {
      try {
        const text = clip(row.input_text, 2000);
        ...
        await sql`
          UPDATE public.learning_examples
        ...
        console.error(`[learning-embed] Error for ${row.id}:`, msg);
      }
    });

    // Count remaining unembedded (useful for backfill loops)
    const [{ count: remaining }] = await sql`
      SELECT count(*)::int as count FROM public.learning_examples
      WHERE company_id = ${companyId} AND embedding IS NULL
    `;
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[learning-embed] Fatal error:", msg);
