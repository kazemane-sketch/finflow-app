// supabase/functions/parse-bank-pdf-ocr/index.ts
// Engine OCR sperimentale (stesso contratto SSE di parse-bank-pdf).
// Nota: in questa prima fase usa la stessa pipeline base del legacy con settaggi separati.
...
const MODEL = Deno.env.get("GEMINI_OCR_MODEL") ?? Deno.env.get("GEMINI_MODEL") ?? "gemini-2.5-flash";
const CHUNK_SIZE = Number(Deno.env.get("PDF_OCR_CHUNK_PAGES") ?? Deno.env.get("PDF_CHUNK_PAGES") ?? "1");
const MAX_OUTPUT_TOKENS = Number(Deno.env.get("GEMINI_OCR_MAX_OUTPUT_TOKENS") ?? Deno.env.get("GEMINI_MAX_OUTPUT_TOKENS") ?? "32768");
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error("parse-bank-pdf-ocr error:", e);
    return new Response(
